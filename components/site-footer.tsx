  ...
  return (
    <footer className="border-t border-[var(--line-soft)] bg-[rgba(7,20,27,0.92)]">
      <div className={`mx-auto grid w-full max-w-[104rem] gap-6 px-4 py-10 md:px-8 ${compact ? "lg:grid-cols-[1.25fr_0.75fr]" : "lg:grid-cols-[1.05fr_0.65fr_0.9fr]"}`}>
        <div>
          <p className="font-display text-[2rem] tracking-[-0.03em] text-[var(--ink-strong)]">Film Forage</p>
          <p className="mt-3 max-w-xl text-sm leading-7 text-[var(--ink-dim)]">
            A cleaner way to land on one good movie, keep a few backups in reach, and verify streaming availability before the group starts wandering.
          </p>
          <p className="mt-3 text-sm text-[var(--ink-muted)]">&copy; {year} Film Forage by Shoaib (Aib) Syed.</p>
          {!compact ? (
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="inline-flex items-center rounded-full border border-[var(--line-soft)] bg-[var(--panel)] px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-[var(--ink-dim)]">Lead movie first</span>
              <span className="inline-flex items-center rounded-full border border-[var(--line-soft)] bg-[var(--panel)] px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-[var(--ink-dim)]">Backup shortlist</span>
              <span className="inline-flex items-center rounded-full border border-[var(--line-soft)] bg-[var(--panel)] px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-[var(--ink-dim)]">Live service checks</span>
            </div>
          ) : null}
        </div>
        ...
        {!compact ? (
          <div>
            ...
            <p className="mt-3 text-sm leading-7 text-[var(--ink-dim)]">
              If live data drops out, Film Forage switches to the reserve shelf and labels it directly instead of pretending the stream data is current.
            </p>
