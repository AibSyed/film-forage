...
export function SiteFooter({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <footer className="border-t border-[var(--line-soft)] bg-[rgba(6,18,25,0.9)]">
      <div className={`mx-auto grid w-full max-w-[92rem] gap-6 px-4 py-10 md:px-8 ${compact ? "lg:grid-cols-[1.2fr_0.8fr]" : "lg:grid-cols-[1.15fr_0.85fr_0.95fr]"}`}>
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm leading-7 text-[var(--ink-dim)]">
            A calmer way to narrow the field, check availability by region, and keep the shortlist close while the room decides.
          </p>
        ...
        <div>
          ...
          <div className="mt-3 grid gap-2 text-sm text-[var(--ink-dim)]">
            <Link href={"/" as Route} className="hover:text-[var(--ink-main)]">Forage</Link>
